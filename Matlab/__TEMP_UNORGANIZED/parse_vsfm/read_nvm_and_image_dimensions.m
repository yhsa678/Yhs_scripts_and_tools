function [camera_data, point_data, point_observations] =...
    ...
    for point_idx = 1:num_points
        ...
        %xyz = fscanf(file, '%f %f %f', 3);
        %rgb = fscanf(file, '%d %d %d', 3);
        %num_observations = fscanf(file, '%d', 1);
        xyz = file_data(1:3);
        rgb = file_data(4:6);
        num_observations = int32(file_data(7));
        
        %camera_indices = zeros(1, num_observations, 'int32');
        %feature_indices = zeros(1, num_observations, 'int32');
        %locations_2d = zeros(2, num_observations);
...
%         for observe_idx = 1:num_observations
%             camera_idx = fscanf(file, '%d', 1);
%             feature_idx = fscanf(file, '%d', 1);
%             location_2d = fscanf(file, '%f %f', 2);
%             camera_idx = camera_idx + 1;
%             feature_idx = feature_idx + 1;
%             location_2d = location_2d + (image_dimensions(:,camera_idx) ./ 2) + 1;
%             
%             camera_indices(observe_idx) = camera_idx;
%             feature_indices(observe_idx) = feature_idx;
%             locations_2d(:,observe_idx) = location_2d;
%         end
